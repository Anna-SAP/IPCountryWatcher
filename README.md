...
优先查询公网 IPv4（api.ipify.org，然后 checkip.amazonaws.com）；两者失败后使用 api64.ipify.org 尝试 IPv4/IPv6。
仅展示一个当前出口，不同时列出所有网卡和双栈地址；切换到 IPv6 备用查询源也可能表现为 IP 变化。

## 国家与国旗

对**已获取的明确 IP**调用 ipwho.is 查询国家，避免使用另一查询服务自己的出口地址误配国旗。
成功的国家结果按 IP 缓存 24 小时；仅在新 IP、缓存过期或失败后的重试窗口到来时查询。
国家查询失败后等待 5 分钟重试，HTTP 429 尊重 Retry-After（未给出时等待 24 小时），其间仍持续监控 IP。
252 张国家 / 地区 / 组织旗帜作为 PNG 内嵌在 EXE 中，运行时无需下载；服务返回未知代码时显示代码占位。
地理信息来自第三方数据库，VPN、代理、地址分配变化会影响准确性；国家标识不等于设备物理位置。

## 网络与本地数据

运行程序会向 ipify / AWS checkip 发起 HTTPS 请求以查询本机公网 IP，向 ipwho.is 提交该 IP 查询国家；这些服务会获知请求来源的公网 IP。
...
测试使用内存中的模拟 HTTP 服务，不向公网接口发送请求；覆盖 IP / JSON 验证、缓存、失败恢复、限流、取消、过期请求、国旗资源与真实 WinForms 消息循环。
